...
export default async function ResidentHelpersPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <AddHelperForm t={t} showCategoryFields={false} />
      </div>
      ...
      {helpers.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {helpers.map((helper) => {
            ...
            return (
              <Link key={helper.id} href={`/resident/helpers/${helper.id}`}>
                <Card className="flex h-full flex-col gap-2 p-4 transition-transform hover:-translate-y-0.5">
                  ...
                  <div className="mt-2 flex flex-wrap gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <div className="font-bold">{outstandingLoan > 0 ? rupees(outstandingLoan) : t.none}</div>
                    </div>
                    {lastSettlement && (
                      <div>
